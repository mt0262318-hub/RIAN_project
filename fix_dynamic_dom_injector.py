...
import re
# Clean up previous injection attempts
code = re.sub(r'<style>[\s\S]*?/\* DYNAMIC MOBILE BOX CSS \*/[\s\S]*?</style>', '', code)
code = re.sub(r'<script>[\s\S]*?Dynamic Mobile Box Injector[\s\S]*?</script>', '', code)

# Robust Dynamic Injector Patch
dynamic_patch = '''
<style>
...
if '</body>' in code:
    code = code.replace('</body>', dynamic_patch + '\n</body>')
    with open('main.py', 'w', encoding='utf-8') as f:
        f.write(code)
    print("SUCCESS: Dynamic DOM injector patched!")
else:
