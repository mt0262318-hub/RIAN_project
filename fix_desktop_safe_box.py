...
import re
# Clean up any previous broken tags
code = re.sub(r'<style>[\s\S]*?/\* SAFE MOBILE BOX \*/[\s\S]*?</style>', '', code)
...
@media screen and (max-width: 768px) {
    /* Hide ONLY mobile-specific redundant elements if needed, leaving desktop completely untouched */
...
if '</body>' in code:
    code = code.replace('</body>', safe_patch + '\n</body>')
    with open('main.py', 'w', encoding='utf-8') as f:
        f.write(code)
    print("SUCCESS: Desktop-safe perfect box injected!")
