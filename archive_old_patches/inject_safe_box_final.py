...
import re
# Clean any previous variations to avoid duplicates
code = re.sub(r'<style>[\s\S]*?#perfect-mobile-box[\s\S]*?</style>', '', code)
code = re.sub(r'<div id="perfect-mobile-box">[\s\S]*?</div>', '', code)

# Exact Safe Box Code Insertion
safe_box_snippet = '''
<style>
...
if '</body>' in code:
    code = code.replace('</body>', safe_box_snippet + '\n</body>')
    with open('main.py', 'w', encoding='utf-8') as f:
        f.write(code)
    print("SUCCESS: Perfect box safely injected!")
else:
