...
import re
# Clean up previous mobile patches
code = re.sub(r'<style>[\s\S]*?@media screen and (max-width: 768px)[\s\S]*?</style>', '', code)
code = re.sub(r'<script>[\s\S]*?window\.addEventListener\(\x27DOMContentLoaded\x27.*?</script>', '', code, flags=re.DOTALL)
...
if '</body>' in code:
    code = code.replace('</body>', final_patch + '\n</body>')
    with open('main.py', 'w', encoding='utf-8') as f:
        f.write(code)
    print("SUCCESS: Exact mockup box patched!")
else:
