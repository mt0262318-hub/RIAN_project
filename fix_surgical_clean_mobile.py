...
import re
# Clean up previous mobile additions safely
code = re.sub(r'<style>[\s\S]*?/\* STRICT MOBILE SCOPE \*/[\s\S]*?</style>', '', code)
code = re.sub(r'<script>[\s\S]*?strict-mobile-shell[\s\S]*?</script>', '', code)

# Surgical Clean Mobile CSS & JS (Keeps Desktop 100% Safe, Fixes Phone Duplication)
surgical_patch = '''
...
@media screen and (max-width: 768px) {
    ...
    /* The exact gorgeous glass card you approved */
    .strict-bottom-card {
